...
def create_subcategory_page(main_cat, sub_cat):
    template_map = {
        ...
        "Vêtements": "vetements_template.html",  # CHANGÉ ICI : Utilise le nouveau template adapté
        "Maison": "maison.html",
    ...
    # Remplacements nécessaires (simplifiés pour le nouveau template)
    content = content.replace("[[SUBCAT]]", sub_cat)
    # Filtre strict sur catégorie + sous-catégorie (pas besoin de re.sub si déjà bon dans le template, mais gardé pour compatibilité avec autres cats)
